...
## 2026-08-26 修订（评审反馈后与实现的差异）

- 主题演示不再使用真实截图：改为纯 CSS 模拟仪表盘（`src/data/themes.ts` 十套色板 + `--dm-*` 变量），十套主题全部可实时切换预览；`public/screenshots/` 已删除。
- Bento「深度图表」卡改用 CSS 柱状图 + 热力图；「多客户端聚合」卡重绘为带标签的流程 SVG。
- 年度回顾 CTA 打开站内 `<dialog>` 示例弹窗（模拟 Wrapped 界面），不再外跳 GitHub。
- Hero NowPlaying 进度条与时间标签由同一 rAF 时间轴驱动，保持一致。
- 视觉降噪：主 CTA 统一纯色橙，去除彩虹渐变按钮/文字渐变/渐变边框卡，光晕与均衡器条减淡；Footer 删除免责声明与站名底行。
